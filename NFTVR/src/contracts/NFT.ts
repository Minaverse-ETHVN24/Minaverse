  ...
  constructor(leafs?: { index: Field; leaf: Field }[]) {
    ...
    if (leafs) {
      for (let i = 0; i < leafs.length; i++) {
        this._tree.setLeaf(leafs[i].index.toBigInt(), leafs[i].leaf);
      }
    }
  }
...
export class NFT extends SmartContract {
  ...
  @method mint(
    ...
    this.ipfsHashRoot.set(
      ipfsHashWitness.calculateRoot(IpfsHashStorage.calculateLeaf(ipfsHash))
    );
  ...
  @method buy(
    ...
    this.ownerRoot.set(
      currentOwnerWitness.calculateRoot(OwnerStorage.calculateLeaf(this.sender))
    );
    this.priceRoot.set(
      priceWitness.calculateRoot(PriceStorage.calculateLeaf(new UInt64(0)))
    );
