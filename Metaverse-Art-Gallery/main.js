...
const ip = "https://192.168.18.1:443";
const imageArr = [0, 4, 5, 6, 7, 8];

async function buyNFT(imageElementId, imageId) {
  const priceElement = document.getElementById(`${imageElementId}Price`);
  const myTransactionElement = document.getElementById(`my-transaction`);

  myTransactionElement.setAttribute(
    "style",
    "display: inline-block;background: lavenderblush;color: #333333;border-radius: 1em;padding: 1em;margin: 0;accent-color: hotpink;",
  );
  ...
  const response = await fetch(`${ip}:443/market/buy`, {
    method: "POST",
    body: JSON.stringify({
      privateKey: "EKEYnGwJpu569dC3m8qAePiUMpEjEbj7GKj9V2KQGdc1oBnRLfp7",
      id: imageId,
    }),
    headers: {
  ...
  console.log(imageElementId, imageId);

  console.log("Sender:", imageElementId, imageId);
  console.log("Response?:", response.status);
  if (response.status == 404 || response.status == 400) {
    priceElement.setAttribute("value", "Failed!");
    const loadingElement = document.getElementById(`spinner`);
    loadingElement.setAttribute("style", "display:none;");

    const transactionElement = document.getElementById(`transactionhash`);
    transactionElement.setAttribute("style", "display:block;");
    transactionElement.innerHTML = "Transaction failed";
  } else {
    priceElement.setAttribute("value", "Bought!");
    const loadingElement = document.getElementById(`spinner`);
    loadingElement.setAttribute("style", "display:none;");
    ...
    transactionElement.innerHTML = rsp;

    console.log("Renewed response:", response);

    const hyperlinkElement = document.getElementById(`hyperlink`);
    hyperlinkElement.setAttribute(
      "onclick",
      `window.open('https://minascan.io/berkeley/tx/${rsp}','_blank')`,
    );
  }
}

AFRAME.registerComponent('buy-nft', {
  init: function () {
    var data = this.data;
    this.el.addEventListener('click', async () => {
      console.log("Registered?");
      await buyNFT(this.data.elementId, this.data.imageId);
    });
  }
});
